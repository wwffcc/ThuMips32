	...
	private void genAsmForBB(BasicBlock bb) {
		for (Tac tac = bb.tacList; tac != null; tac = tac.next) {
			...
			case SUB:
				...
				break;
			case MUL:
				bb.appendAsm(new MipsAsm(MipsAsm.FORMAT2, "mult",
						tac.op1.reg, tac.op2.reg));
				bb.appendAsm(new MipsAsm(MipsAsm.FORMAT1,"mflo",tac.op0.reg));
				break;
			case DIV:
				bb.appendAsm(new MipsAsm(MipsAsm.FORMAT3, "div", "$zero",
						tac.op1.reg, tac.op2.reg));
				bb.appendAsm(new MipsAsm(MipsAsm.FORMAT1,"mflo",tac.op0.reg));
				break;
			case MOD:
			...
			case DIRECT_CALL:
				...
				break;
			case PARM:
				int valx = tac.op1.value/4-1;
				if(valx<4){
					bb.appendAsm(new MipsAsm(MipsAsm.FORMAT2,"move","$a"+valx,tac.op0.reg));
				}
